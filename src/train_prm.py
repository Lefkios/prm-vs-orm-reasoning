...
CONFIG = {
    ...
    "output_path": "models/prm-human-trained",
    ...
    "wandb_run_name": "prm-human-training",
    "lora_r": 16,
    "lora_alpha": 32,
    "lora_dropout": 0.05,
}

# globals set up once in main() so the reward function (which TRL calls
# with a fixed signature) can access the PRM classifier and tokenizer
PRM_MODEL = None
PRM_TOKENIZER = None
...
def split_into_steps(text):
    """Split a model completion into reasoning steps for step-level scoring.
    Solutions in this project use newlines between steps, so split on those
...
def score_steps(prompt, steps):
    """Run the PRM classifier over each prefix of steps and return the
    average P(correct) across all steps — this is the per-completion reward."""
    if not steps:
        return 0.0
    ...
    with torch.no_grad():
        ...
        probs = F.softmax(logits, dim=-1)[:, 1]  # P(label == 1, i.e. correct)
...
def prm_reward_function(prompts, completions, answer, **kwargs):
    ...
    for prompt, completion, true_answer in zip(prompts, completions, answer):
        ...
        step_score = score_steps(prompt_text, steps)  # in [0, 1]

        predicted = extract_answer(text)
        outcome_bonus = 0.5 if predicted == true_answer else -0.5

        # blend: step quality drives most of the signal, outcome nudges it
...
def main():
    ...
    # the classifier was saved as a LoRA adapter (+ modules_to_save for the
    # score head), not a merged model — load the base model explicitly and
    # apply the adapter, rather than AutoModelForSequenceClassification's
    # automatic adapter loading, which doesn't pick up modules_to_save
    # correctly and silently leaves the trained classification head untrained
    ...
    print("PRM-human training complete.")
